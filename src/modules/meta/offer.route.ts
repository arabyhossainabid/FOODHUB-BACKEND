...
const getAllOffers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // If no offers in DB, seed some initial ones and return them
    if (offers.length === 0) {
      const initialOffers = [
        {
      ...
      const seededOffers = await prisma.offer.findMany({ where: { isActive: true } });
      return res.status(200).json({ success: true, data: seededOffers });
    }
